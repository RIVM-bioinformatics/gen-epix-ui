...
import { useIdentifierIssuerOwnOrganizationOptionsQuery } from '@gen-epix/ui-client-common/dataHooks/useIdentifierIssuerOwnOrganizationQuery';
...
export const UploadMapColumns = () => {
  ...
  const schema = useMemo(() => {
    return UploadUtil.getColumnMappingSchema(rawData, completeCaseType);
  }, [completeCaseType, rawData]);

  const defaultValues: UploadMappedColumnsFormFields = useMemo(() => {
    return UploadUtil.getDefaultColumnMappingFormValues(rawData[0], store.getState().mappedColumns, identifierIssuerOptionsQuery.options);
  }, [rawData, store, identifierIssuerOptionsQuery.options]);

  const formMethods = useForm<UploadMappedColumnsFormFields>({
  ...
  const formFieldDefinitions = useMemo<FormFieldDefinition<UploadMappedColumnsFormFields>[]>(() => {
    return UploadUtil.getColumnMappingFormFieldDefinitions(completeCaseType, rawData[0], fileName).map(def => ({
      ...def,
  ...
  useEffect(() => {
    const perform = async () => {
      await setMappedColumns(UploadUtil.getMappedColumnsFromFormData(columnMappingFormValues, rawData, colMap.map, completeCaseType));
    };
  ...
  const unMappedColumns = useMemo(() => {
    ...
    const mappedIndicies = UploadUtil.getMappedColumnsFromFormData(columnMappingFormValues, rawData, colMap.map, completeCaseType).map(mappedColumn => mappedColumn.originalIndex);
  ...
  const onFormSubmit = useCallback(async (data: UploadMappedColumnsFormFields) => {
    await setMappedColumns(UploadUtil.getMappedColumnsFromFormData(data, rawData, colMap.map, completeCaseType));
    await goToNextStep();
  }, [rawData, colMap.map, goToNextStep, setMappedColumns, completeCaseType]);

  const sampleIdColIds = useMemo(() => {
    return UploadUtil.getSampleIdColIds(completeCaseType);
  }, [completeCaseType]);

  const renderField = useCallback((definition: FormFieldDefinition<UploadMappedColumnsFormFields>, element: ReactElement) => {
    ...
    <ResponseHandler loadables={loadables}>
      ...
      >
        ...
        >
          ...
          >
            <Container>
              <GenericForm<UploadMappedColumnsFormFields>
