...
import type { TableStore } from '@gen-epix/ui-client-common/stores/tableStore';
import {
  Table,
  TableMenu,
...
import { CaseContentFormDialog } from '../CaseContentFormDialog';

import type { UploadEditColumnValuesDialogRefMethods } from './UploadEditColumnValuesDialog';
import { UploadEditColumnValuesDialog } from './UploadEditColumnValuesDialog';


export type UploadCaseResultTableProps = {
  ...
  readonly getOriginalCellValue: (col: CaseDbCol, cellParams: TableRowAndColumnParams<CaseDbCaseUploadResult, null>, issue: CaseDbCaseDataIssue) => string;
  ...
  readonly tableStore: StoreApi<TableStore<CaseDbCaseUploadResult>>;
};

export const UploadCaseResultTable = ({ caseRightsColMap, caseUploadResults, completeCaseType, getOriginalCellValue, onCaseContentEditSubmit, onColContentEditSubmit, tableStore }: UploadCaseResultTableProps) => {
  const theme = useTheme();
  const { t } = useTranslation();
  const caseFormDialogRef = useRef<CaseContentFormDialogRefMethods>(null);
  const uploadEditColumnValuesDialogRef = useRef<UploadEditColumnValuesDialogRefMethods>(null);
  ...
  const onTableCustomHeaderIconClick = useCallback((params: TableColumnParams<CaseDbCaseUploadResult, null>) => {
    uploadEditColumnValuesDialogRef.current?.open({
  ...
  const renderHasIssueCell = useCallback(({ row }: TableRowParams<CaseDbCaseUploadResult>) => {
  ...
  const renderIsNewCell = useCallback(({ row }: TableRowParams<CaseDbCaseUploadResult>) => {
    return row.id ? null : (
      <AddIcon
        aria-hidden={false}
  ...
  const renderCell = useCallback(({ id, row }: TableRowParams<CaseDbCaseUploadResult>) => {
  ...
  const tableColumns = useMemo<TableColumn<CaseDbCaseUploadResult>[]>(() => {
    const tableCols: TableColumn<CaseDbCaseUploadResult>[] = [];
    ...
    completeCaseType.ordered_col_ids.forEach((colId) => {
      ...
      if (col) {
        tableCols.push({
          ...
          widthPx: 250,
        } satisfies TableColumn<CaseDbCaseUploadResult>);
      }
    });
    ...
    tableCols.push(TableUtil.createActionsColumn({
      getActions: (params: TableRowParams<CaseDbCaseUploadResult>) => {
        ...
        actions.push(
          <MenuItem
            ...
            onClick={() => {
              caseFormDialogRef.current?.open({
  ...
  useInitializeTableStore<CaseDbCaseUploadResult>({ columns: tableColumns, createFiltersFromColumns: true, rows: caseUploadResults, store: tableStore });
  ...
  return (
    <>
      ...
      <Table
        font={theme['gen-epix-ui-client-casedb'].lineList.font}
      />
      <CaseContentFormDialog
        onSubmit={onCaseContentFormDialogSubmit}
        ref={caseFormDialogRef}
      />
      <UploadEditColumnValuesDialog
        onSubmit={onEpiUploadEditColumnValuesDialogSubmit}
        ref={uploadEditColumnValuesDialogRef}
      />
    </>
  );
};
