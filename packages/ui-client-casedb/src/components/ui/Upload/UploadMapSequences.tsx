...
export const UploadMapSequences = () => {
  ...
  const completeCaseTypeColStats = useMemo(() => {
    return UploadUtil.getCompleteCaseTypeColStats(completeCaseType);
  }, [completeCaseType]);

  const alertTitleId = useId();
  const alertContentId = useId();

  const uploadSequenceMappingRef = useRef(store.getState().sequenceMapping);

  const onProceedButtonClick = useCallback(async () => {
    setSequenceMapping(uploadSequenceMappingRef.current);
    await goToNextStep();
  ...
  const updateAlert = useCallback(() => {
    ...
    const {
      ...
      numberOfFilesToMap,
      unmappedFileNames,
    } = UploadUtil.getSequenceMappingStats(uploadSequenceMappingRef.current, sequenceFilesDataTransfer);
  ...
  const caseHasColumnContent = useCallback((rows: CaseDbCaseUploadResult[], col: CaseDbCol): boolean => {
  ...
  const renderSequenceCell = useCallback((tableRowParams: TableRowParams<CaseDbCaseUploadResult>) => {
    const col = completeCaseType.cols[tableRowParams.id];

    const id = tableRowParams.row.id;
    const dropDownValue = uploadSequenceMappingRef.current?.[tableRowParams.row.id]?.sequenceFileNames[col.id] || '';
    const onChange = (newValue: string) => {
      if (!newValue) {
        delete uploadSequenceMappingRef.current?.[id]?.sequenceFileNames[col.id];
      } else {
        uploadSequenceMappingRef.current[id].sequenceFileNames[col.id] = newValue;
      }
    };
  ...
  const renderReadsCell = useCallback((tableRowParams: TableRowParams<CaseDbCaseUploadResult>) => {
    const col = completeCaseType.cols[tableRowParams.id];
    const isSequenceColumn = completeCaseTypeColStats.sequenceColumns.includes(col);

    const id = tableRowParams.row.id;
    const dropDownValueFwd = uploadSequenceMappingRef.current?.[id]?.readsFileNames?.[col.id]?.fwd || '';
    const dropDownValueRev = uploadSequenceMappingRef.current?.[id]?.readsFileNames?.[col.id]?.rev || '';
    const onChangeFwd = (newValue: string) => {
      if (!newValue) {

        delete uploadSequenceMappingRef.current?.[id]?.readsFileNames?.[col.id]?.fwd;
      } else {
        uploadSequenceMappingRef.current[id].readsFileNames[col.id].fwd = newValue;
      }
    };
    const onChangeRev = (newValue: string) => {
      if (!newValue) {

        delete uploadSequenceMappingRef.current?.[id]?.readsFileNames?.[col.id]?.rev;
      } else {
        uploadSequenceMappingRef.current[id].readsFileNames[col.id].rev = newValue;
      }
    };
