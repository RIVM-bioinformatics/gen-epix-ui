...
export interface AddCasesToEventDialogOpenProps {
  currentCaseSet: CaseDbCaseSet;
  rows: CaseDbCase[];
}

export interface AddCasesToEventDialogProps extends WithDialogRenderProps<AddCasesToEventDialogOpenProps> {
  //
}

export type AddCasesToEventDialogRefMethods = WithDialogRefMethods<AddCasesToEventDialogProps, AddCasesToEventDialogOpenProps>;
...
export const AddCasesToEventDialog = withDialog<AddCasesToEventDialogProps, AddCasesToEventDialogOpenProps>((
  {
    onActionsChange,
    onClose,
    onTitleChange,
    openProps,
  }: AddCasesToEventDialogProps,
): ReactElement => {
  ...
  const similarCaseIds = useStore(dashboardStore, useShallow((state) => state.findSimilarCasesResults?.flatMap(result => result.similarCaseIds) ?? []));
  const formId = useId();
  ...
  const { isMutating: isMutatingItems, mutate: mutateItems } = useEditMutation<CaseDbCaseSetMember[]>({
    ...
    getSuccessNotificationMessage: () => (
      <AddCasesToEventDialogSuccessNotificationMessage
  ...
  useEffect(() => {
    ...
    actions.push(
      {
        ...TestIdUtil.createAttributes('AddCasesToEventDialog-confirmButton'),
        autoFocus: true,
        color: 'secondary',
