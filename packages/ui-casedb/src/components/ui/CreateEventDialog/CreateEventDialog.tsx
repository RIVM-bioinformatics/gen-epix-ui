...
export interface CreateEventDialogOpenProps {
  readonly completeCaseType?: CaseDbCompleteCaseType;
  readonly rows?: CaseDbCase[];
}

export interface CreateEventDialogProps extends WithDialogRenderProps<CreateEventDialogOpenProps> {
}

export type CreateEventDialogRefMethods = WithDialogRefMethods<CreateEventDialogProps, CreateEventDialogOpenProps>;
...
export const CreateEventDialog = withDialog<CreateEventDialogProps, CreateEventDialogOpenProps>((
  {
    onActionsChange,
    onClose,
    onTitleChange,
  ...
  const onSuccess = useCallback(async (item: CaseDbCaseSet, variables: FormFields) => {
    ...
    EventBusService.getInstance().emit('onEventCreated');
    onClose();
  ...
  const { isMutating: isCreating, mutate: mutateCreate } = useCreateMutation<CaseDbCaseSet, FormFields>({
    ...
    getSuccessNotificationMessage: (item) => (
      <CreateEventDialogSuccessNotificationMessage
        caseSet={item}
        isCreating
      />
    ),
  ...
  useEffect(() => {
    ...
    if (isLoading || !isFormDisabled) {
      actions.push({
        ...TestIdUtil.createAttributes('CreateEventDialog-saveButton'),
        autoFocus: true,
        color: 'secondary',
