...
export const CaseSummary = ({ caseDbCase }: CaseSummaryProps) => {
  ...
  const numVisibleAttributesInSummary = useStore(dashboardStore, (state) => state.numVisibleAttributesInSummary);
  const setNumVisibleAttributesInSummary = useStore(dashboardStore, (state) => state.setNumVisibleAttributesInSummary);
    ...
    <Box>
      ...
      >
        {visibleAttributes.map(tableColumn => {
          try {
            const value = CaseUtil.getRowValue(caseDbCase.content, tableColumn.columnContext, completeCaseType);
            return (
