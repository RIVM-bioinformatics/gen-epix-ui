...
export const CaseSetForm = ({ caseSet, formId, onFinish, onIsSavingChange }: CaseSetFormProps) => {
  ...
  const { isMutating: isEditing, mutate: mutateEdit, setPreviousItem } = useEditMutation<CaseDbCaseSet, FormFields>({
    ...
    getSuccessNotificationMessage: (item) => <CreateEventDialogSuccessNotificationMessage caseSet={item} />,
    onError,
    onSuccess,
