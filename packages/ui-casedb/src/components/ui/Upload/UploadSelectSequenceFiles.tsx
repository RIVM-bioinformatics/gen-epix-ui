...
export const UploadSelectSequenceFiles = () => {
  ...
  const hasGenomeFiles = useMemo(() => {
    return selectedFiles.some(file => UploadUtil.isGenomeFile(file.name));
  }, [selectedFiles]);
  const hasReadsFiles = useMemo(() => {
    return selectedFiles.some(file => UploadUtil.isReadsFile(file.name));
  }, [selectedFiles]);
