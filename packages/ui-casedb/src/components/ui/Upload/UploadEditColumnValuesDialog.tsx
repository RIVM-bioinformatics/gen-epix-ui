...
export interface UploadEditColumnValuesDialogOpenProps {
  colId: string;
  completeCaseType: CaseDbCompleteCaseType;
  shouldShowRightsWarning?: boolean;
}

export interface UploadEditColumnValuesDialogProps extends WithDialogRenderProps<UploadEditColumnValuesDialogOpenProps> {
  onSubmit: (content: CaseDbCase['content']) => void;
}

export type UploadEditColumnValuesDialogRefMethods = WithDialogRefMethods<UploadEditColumnValuesDialogProps, UploadEditColumnValuesDialogOpenProps>;


export const UploadEditColumnValuesDialog = withDialog<UploadEditColumnValuesDialogProps, UploadEditColumnValuesDialogOpenProps>((
  {
    onActionsChange,
    onClose,
    onSubmit,
    onTitleChange,
    openProps,
  }: UploadEditColumnValuesDialogProps,
): ReactElement => {
  ...
  useEffect(() => {
    const actions: DialogAction[] = [
      {
        ...TestIdUtil.createAttributes('UploadEditColumnValuesDialog-submitButton'),
        color: 'primary',
        form: formId,
  ...
  fullWidth: true,
  maxWidth: 'md',
  testId: 'UploadEditColumnValuesDialog',
});
