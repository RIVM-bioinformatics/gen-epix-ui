...
import type { GenEpixPieSeriesOptionEventData } from '../../../utils/MapUtil';
...
export const MapWidget = () => {
  ...
  const mapWidgetData = useStore(dashboardStore, (state) => state.getWidgetData<MapWidgetData>(DASHBOARD_COMPONENT_NAME.MAP));
  ...
  useEffect(() => {
    ...
    if (mapWidgetData.columnId) {
      preferredCol = CaseTypeUtil.getCols(completeCaseType).find(c => c.id === mapWidgetData.columnId);
    } else {
      preferredCol = CaseTypeUtil.getPreferredGEOCol(CaseTypeUtil.getCols(completeCaseType, geoDims[0].id));
    }
    setCol(preferredCol);
  }, [col, completeCaseType, mapWidgetData.columnId, geoDims]);
  ...
  const regions = useMemo(() => {
    ...
    return DataService.getInstance().data.regionsByRegionSetId[regionSetId];
  ...
  const maxPieChartArea = useMemo(() => MapUtil.getMaxPieChartArea(width, height, numZones), [height, numZones, width]);

  const lineListRegionStatistics = useMemo(() => MapUtil.getRegionStatistics(sortedData, col?.id, regions), [col, sortedData, regions]);
  const getPieChartRadius = useCallback((numCases: number): number => {
    return MapUtil.getPieChartRadius(numCases, maxPieChartArea, lineListRegionStatistics);
  }, [lineListRegionStatistics, maxPieChartArea]);

  const { mapCaseCount, series } = useMemo(
    () => MapUtil.getMapSeries(col, regions, sortedData, stratification, getPieChartRadius),
  ...
  const getOptions = useCallback(() => {
    ...
    const aspectScale = MapUtil.getGeoJsonAspectScale(regionSetShape.geo_json);
    ...
    return {
      mouseout: () => {
        dashboardContext.highlight({
          caseIds: [],
          origin: DASHBOARD_COMPONENT_NAME.MAP,
        });
      },
  ...
  }, [chartRef, dashboardContext.highlightSubject, series]);
    ...
    completeCaseType.ordered_dim_ids.map(x => completeCaseType.dims[x]).filter(dim => {
      ...
      completeCaseType.ordered_col_ids_by_dim[dim.id].map(id => completeCaseType.cols[id]).forEach((c) => {
        menu.items.push({
          active: !!(c.id === col?.id),
          callback: () => {
            updateWidgetData<MapWidgetData>(DASHBOARD_COMPONENT_NAME.MAP, { columnId: c.id });
            setCol(c);
          },
          label: c.label,
  ...
  useEffect(() => {
    const emitDownloadOptions = () => {
      EventBusService.getInstance().emit('onDownloadOptionsChanged', {
        ...
        zone: DASHBOARD_COMPONENT_NAME.MAP,
        zoneLabel: t`Map`,
      });
    };
