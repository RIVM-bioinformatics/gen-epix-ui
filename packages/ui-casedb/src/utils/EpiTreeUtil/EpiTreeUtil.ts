...
export class EpiTreeUtil {
   ...
   */
  public static drawDivider(params: { canvas: HTMLCanvasElement; devicePixelRatio: number; y: number }): void {
    const { canvas, devicePixelRatio, y } = params;
  ...
  public static drawGuides(params: { canvas: HTMLCanvasElement; devicePixelRatio: number; endY?: number; geneticTreeWidth: Decimal; horizontalScrollPosition: number; paddingBottom?: number; paddingTop?: number; pixelToGeneticDistanceRatio: number; startY?: number; tickerMarkScale: TickerMarkScale; zoomLevel: number }): void {
    ...
    EpiTreeUtil.draw(canvas, devicePixelRatio, (ctx) => {
      ctx.strokeStyle = ConfigManager.getInstance().config.epiTree.REGULAR_FILL_COLOR_SUPPORT_LINE;
      ctx.setLineDash([3, 1]);
    ...
    EpiTreeUtil.draw(canvas, devicePixelRatio, (ctx) => {
      ...
      EpiTreeUtil.forEachScaleLine({
        callback: (x, i, numberOfLines) => {
          ...
          ctx.fillText(NumberUtil.toStringWithPrecision(label, tickerMarkScale[2]), x, ConfigManager.getInstance().config.epiTree.HEADER_HEIGHT * 0.61);
          ctx.closePath();
        },
        devicePixelRatio,
        geneticTreeWidth,
  ...
  public static drawTreeCanvas(params: {
    ...
    EpiTreeUtil.drawBackground({ canvas, devicePixelRatio, theme, treeCanvasHeight: treeCanvasHeight + headerHeight, treeCanvasWidth });
    EpiTreeUtil.drawGuides({ canvas, devicePixelRatio, endY: headerHeight + treeCanvasHeight, geneticTreeWidth, horizontalScrollPosition, pixelToGeneticDistanceRatio, startY: headerHeight, tickerMarkScale, zoomLevel });

    if (headerHeight > 0) {
      EpiTreeUtil.drawGuides({ canvas, devicePixelRatio, endY: headerHeight, geneticTreeWidth, horizontalScrollPosition, pixelToGeneticDistanceRatio, startY: headerHeight * 0.7, tickerMarkScale, zoomLevel });
      EpiTreeUtil.drawGuides({ canvas, devicePixelRatio, endY: headerHeight * 0.3, geneticTreeWidth, horizontalScrollPosition, pixelToGeneticDistanceRatio, startY: 0, tickerMarkScale, zoomLevel });
      EpiTreeUtil.drawScale({ canvas, devicePixelRatio, geneticTreeWidth, horizontalScrollPosition, pixelToGeneticDistanceRatio, theme, tickerMarkScale, zoomLevel });
      EpiTreeUtil.drawDivider({ canvas, devicePixelRatio, y: 0 });
      EpiTreeUtil.drawDivider({ canvas, devicePixelRatio, y: headerHeight - 1 });
    }

    EpiTreeUtil.drawTree({ canvas, devicePixelRatio, externalRange, externalScrollPosition, headerHeight, highlightedNodeNames, horizontalScrollPosition, isLinked, itemHeight, shouldShowDistances, shouldShowSupportLinesWhenUnlinked, stratification, theme, treeAssembly, verticalScrollPosition, zoomLevel });
  }
   ...
   */
  public static getNewScrollPositionForZoomLevel(params: { currentZoomLevel: number; dimensionSize: number; eventOffset: number; newZoomLevel: number; scrollPosition: number }): number {
    const { currentZoomLevel, dimensionSize, eventOffset, newZoomLevel, scrollPosition } = params;
  ...
  public static getSanitizedScrollPosition(params: { devicePixelRatio: number; internalZoomLevel: number; isLinked: boolean; positionX: number; positionY: number; treeCanvasHeight: number; treeCanvasWidth: number; treeHeight: number }): { newPositionX: number; newPositionY: number } {
    ...
    const relativeTreePadding = ((ConfigManager.getInstance().config.epiTree.TREE_PADDING) * devicePixelRatio);
    ...
    if (isLinked && internalZoomLevel === 1) {
      ...
      positionYMin = (-treeCanvasHeight * devicePixelRatio) + relativeTreePadding + (ConfigManager.getInstance().config.epiTree.HEADER_HEIGHT * devicePixelRatio);
      positionYMax = ((treeHeight / internalZoomLevel) * devicePixelRatio) - relativeTreePadding - (ConfigManager.getInstance().config.epiTree.HEADER_HEIGHT * devicePixelRatio);
    }
  ...
  public static getScrollPositionFromTreeVisibility(kwArgs: {
    ...
    const { itemHeight, treeCanvasHeight, treeHeight, treeSize, verticalScrollPosition, zoomLevel } = kwArgs;
  ...
  public static getTickMarkScale(params: { geneticTreeWidth: Decimal; minGeneticScaleUnit: number; treeWidthMinusPadding: number; zoomLevel: number }): TickerMarkScale {
    ...
    let minNumLines = Math.floor(width / ConfigManager.getInstance().config.epiTree.MAX_SCALE_WIDTH_PX) + 1;
    let maxNumLines = Math.ceil(width / ConfigManager.getInstance().config.epiTree.MIN_SCALE_WIDTH_PX) + 1;
    ...
    const multipliedIncrements = ConfigManager.getInstance().config.epiTree.SCALE_INCREMENTS.map(i => new Decimal(i).times(multiplier));
    ...
    geneticDistanceCols.forEach(col => {
      ...
      treeAlgorithms.forEach(treeAlgorithm => {
        treeConfigurations.push({
          col,
          computedId: EpiTreeUtil.getTreeConfigurationId({ col, geneticDistanceProtocol, refCol, treeAlgorithm }),
  ...
  private static assembleAncestorNode(treeAssemblyContext: TreeAssemblyContext, node: TreeNode, childRenderResults: NodeAssemblyResult[]): NodeAssemblyResult {
    ...
    if (node.children.every(child => (child.branchLength?.toNumber() ?? 0) > 0)) {
      ...
      circlePath.arc(ancestorXPxEnd, ancestorYPx, ConfigManager.getInstance().config.epiTree.ANCESTOR_DOT_RADIUS, 0, 2 * Math.PI, false);
  ...
  private static assembleLeafNode(treeAssemblyContext: TreeAssemblyContext, node: TreeNode, distance = 0, leafIndex = 0, externalSortingIndex = 0): NodeAssemblyResult {
    ...
    const leafXPxEnd = leafX * treeAssemblyContext.pixelToGeneticDistanceRatio + ConfigManager.getInstance().config.epiTree.TREE_PADDING;
    ...
    const circlePath = new Path2D();
    circlePath.arc(leafXPxEnd, leafYPx, ConfigManager.getInstance().config.epiTree.LEAF_DOT_RADIUS, 0, 2 * Math.PI, false);
  ...
  private static forEachScaleLine(params: { callback: (x: number, i: number, numberOfLines: number) => void; devicePixelRatio: number; geneticTreeWidth: Decimal; horizontalScrollPosition?: number; pixelToGeneticDistanceRatio: number; tickerMarkScale: TickerMarkScale; zoomLevel: number }): void {
    ...
    for (let i = 0; i < numberOfLines; i++) {
      const x = new Decimal(i).times(tickerWidth).plus(new Decimal(ConfigManager.getInstance().config.epiTree.TREE_PADDING).div(zoomLevel)).minus(offset).minus(new Decimal(horizontalScrollPosition).div(devicePixelRatio)).toNumber();
      callback(x, i, numberOfLines);
    }
  }
  ...
  private static getDistanceLabel(treeAssemblyContext: TreeAssemblyContext, branchLength: Decimal): string {
    ...
    if (branchLength.div(treeAssemblyContext.rootNode.maxBranchLength).mul(100).lessThan(ConfigManager.getInstance().config.epiTree.MINIMUM_DISTANCE_PERCENTAGE_TO_SHOW_LABEL)) {
      return null;
    }
