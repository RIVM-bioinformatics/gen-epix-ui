...
export class FilterUtil {
  ...
  public static createCategoricalFilter(col: CaseDbCol, dimId: string, completeCaseType: CaseDbCompleteCaseType): MultiSelectFilter | TextFilter {
    ...
    if ((refCol.col_type === CaseDbColType.NOMINAL || refCol.col_type === CaseDbColType.ORDINAL || refCol.col_type === CaseDbColType.INTERVAL) && DataService.getInstance().data.conceptsBySetId[refCol.concept_set_id]) {
      const options = DataService.getInstance().data.conceptsBySetId[refCol.concept_set_id].map<AutoCompleteOption>(concept => ({
    ...
    return FilterUtil.createTextFilter(col, dimId);
  }
  ...
  public static createFilterDimensions(completeCaseType: CaseDbCompleteCaseType): FilterDimension[] {
    ...
    completeCaseType.ordered_dim_ids.forEach(dimId => {
      ...
      if (refCols.every(refCol => FilterUtil.colTypeBlackList.has(refCol.col_type))) {
        return;
      }
  ...
  public static createFilters(completeCaseType: CaseDbCompleteCaseType): Filters {
    ...
    completeCaseType.ordered_dim_ids.forEach((dimId) => {
      ...
      cols.forEach(col => {
        const refCol = completeCaseType.ref_cols[col.ref_col_id];
        if (FilterUtil.colTypeBlackList.has(refCol.col_type)) {
          return;
        }
        ...
        if (refDim.dim_type === CaseDbDimType.TIME) {
          ...
          filters.push(
            new DateFilter({
              ...
              dateParser: FilterUtil.getDateParser(refCol),
              filterDimensionId: dimId,
              filterMode: FILTER_MODE.BACKEND,
              filterPriority: DEFAULT_FILTER_GROUP,
              id: col.id,
          ...
          const options = (DataService.getInstance().data.regionsByRegionSetId[refCol.region_set_id]?.map<AutoCompleteOption>(region => {
            return {
          ...
          filters.push(FilterUtil.createCategoricalFilter(col, dimId, completeCaseType));
          ...
          const options = DataService.getInstance().data.organizations.map<AutoCompleteOption>(organization => {
            return {
