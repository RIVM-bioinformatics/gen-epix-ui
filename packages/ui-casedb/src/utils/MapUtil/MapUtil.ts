...
export class MapUtil {
  public static getGeoJsonAspectScale(geoJson: string) {
    const bounds = MapUtil.getGeoJsonBounds(geoJson);
    if (!bounds) {
      return 1;
    }
  ...
  public static getMapSeries(
  ...
  ): { mapCaseCount: number; series: PieSeriesOption[] } {
    if (!col || !regions) {
      return {
        mapCaseCount: undefined,
        series: [],
      };
    }
    ...
    const { numCases: totalNumCases, statisticsPerRegion } = MapUtil.getRegionStatistics(sortedData, col.id, regions);
  ...
  public static getPieChartRadius(numCases: number, maxPieChartArea: number, statistics: RegionStatistics) {
    ...
    const { MIN_PIE_CHART_RADIUS } = ConfigService.getInstance<CaseDbConfig>().config.map;
  ...
  private static expandBoundsFromCoordinates(bounds: GeoJsonBounds, coordinates: unknown) {
    ...
    coordinates.forEach(childCoordinates => MapUtil.expandBoundsFromCoordinates(bounds, childCoordinates));
  }
  ...
  private static expandBoundsFromFeature(bounds: GeoJsonBounds, feature?: GeoJsonFeature | null) {
    ...
    const bboxBounds = MapUtil.normalizeBounds(feature.bbox);
    if (bboxBounds) {
      MapUtil.mergeBounds(bounds, bboxBounds);
      return;
    }

    MapUtil.expandBoundsFromGeometry(bounds, feature.geometry);
  }
  ...
  private static expandBoundsFromGeometry(bounds: GeoJsonBounds, geometry?: GeoJsonGeometry | null) {
    ...
    const bboxBounds = MapUtil.normalizeBounds(geometry.bbox);
    if (bboxBounds) {
      MapUtil.mergeBounds(bounds, bboxBounds);
      return;
    }

    MapUtil.expandBoundsFromCoordinates(bounds, geometry.coordinates);
    geometry.geometries?.forEach(childGeometry => MapUtil.expandBoundsFromGeometry(bounds, childGeometry));
  }

  private static getGeoJsonBounds(geoJson: string): GeoJsonBounds | null {
    try {
      const parsedGeoJson = JSON.parse(geoJson) as GeoJsonObject;
      return MapUtil.getGeoJsonObjectBounds(parsedGeoJson);
    } catch {
      return null;
    }
  }
  ...
  private static getGeoJsonObjectBounds(geoJson: GeoJsonObject): GeoJsonBounds | null {
    const bboxBounds = MapUtil.normalizeBounds(geoJson.bbox);
    if (bboxBounds) {
      return bboxBounds;
    }

    const bounds = MapUtil.createEmptyBounds();
    MapUtil.expandBoundsFromCoordinates(bounds, geoJson.coordinates);
    geoJson.features?.forEach(feature => MapUtil.expandBoundsFromFeature(bounds, feature));
    geoJson.geometries?.forEach(geometry => MapUtil.expandBoundsFromGeometry(bounds, geometry));
    MapUtil.expandBoundsFromGeometry(bounds, geoJson.geometry);

    return MapUtil.hasBounds(bounds) ? bounds : null;
  }
