...
export class EpiLineListCaseSetMembersService {
  private static __instance: EpiLineListCaseSetMembersService;
  ...
  public static getInstance(): EpiLineListCaseSetMembersService {
    EpiLineListCaseSetMembersService.__instance = HmrUtil.getHmrSingleton('epiLineListCaseSetMembersService', EpiLineListCaseSetMembersService.__instance, () => new EpiLineListCaseSetMembersService());
    return EpiLineListCaseSetMembersService.__instance;
  }
  ...
  public async loadRange(caseIds: string[]): Promise<void> {
    const cache = QueryClientService.getInstance().getValidQueryData<EpiCaseHasCaseSet>([CASEDB_QUERY_KEY.XXX_CASE_ID_HAS_CASE_SET]) ?? {};
    ...
    try {
      ...
      queryClient.setQueryData<EpiCaseHasCaseSet>(QueryClientService.getInstance().getGenericKey(CASEDB_QUERY_KEY.XXX_CASE_ID_HAS_CASE_SET), {
        ...newCache,
      });
  ...
  private getItemFromCache(caseId: string): boolean {
    return QueryClientService.getInstance().getValidQueryData<EpiCaseHasCaseSet>([CASEDB_QUERY_KEY.XXX_CASE_ID_HAS_CASE_SET])?.[caseId];
  }
}
