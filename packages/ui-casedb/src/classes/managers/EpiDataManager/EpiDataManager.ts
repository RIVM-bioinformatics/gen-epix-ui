...
export class EpiDataManager {
  ...
  public async loadConcepts(signal: AbortSignal): Promise<void> {
    ...
    let conceptSets = QueryClientManager.getInstance().getValidQueryData<CaseDbConceptSet[]>(QueryClientManager.getInstance().getGenericKey(CASEDB_QUERY_KEY.CONCEPT_SETS)) ?? null;
    let concepts = QueryClientManager.getInstance().getValidQueryData<CaseDbConcept[]>(QueryClientManager.getInstance().getGenericKey(CASEDB_QUERY_KEY.CONCEPTS)) ?? null;
  ...
  public async loadMissingRegions(completeCaseType: CaseDbCompleteCaseType, signal: AbortSignal): Promise<void> {
    ...
    const currentRegionSets = QueryClientManager.getInstance().getValidQueryData<CaseDbRegionSet[]>(QueryClientManager.getInstance().getGenericKey(CASEDB_QUERY_KEY.REGION_SETS_LAZY)) ?? [];
    const currentRegions = QueryClientManager.getInstance().getValidQueryData<CaseDbRegion[]>(QueryClientManager.getInstance().getGenericKey(CASEDB_QUERY_KEY.REGIONS_LAZY)) ?? [];
  ...
  public async loadOrganizations(signal: AbortSignal): Promise<void> {
    ...
    let organizations = QueryClientManager.getInstance().getValidQueryData<CaseDbOrganization[]>(QueryClientManager.getInstance().getGenericKey(COMMON_QUERY_KEY.ORGANIZATIONS)) ?? null;
  ...
  public async loadTreeAlgorithms(completeCaseType: CaseDbCompleteCaseType, signal: AbortSignal): Promise<void> {
    ...
    const queryKey = QueryClientManager.getInstance().getGenericKey(CASEDB_QUERY_KEY.TREE_ALGORITHMS);

    const currentTreeAlgorithms = QueryClientManager.getInstance().getValidQueryData<CaseDbTreeAlgorithm[]>(queryKey);
